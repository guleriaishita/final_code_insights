...
const executePythonScript = async (inputData, tempDir) => {
  const scriptPath = path.resolve(__dirname, '../util/documentation.py');
...
router.post('/generate_documentation', (req, res) => {
  let tempDir;
  let processId;
  ...
  upload(req, res, async (err) => {
    try {
      ...
      processId = `documentation_${Date.now()}`;
      tempDir = path.join(os.tmpdir(), 'docgen', `process_${Date.now()}`);
      
      console.log('Starting documentation generation:', { processId, tempDir });
      ...
      logError(error, 'generate_documentation');
...
// Backend route
router.get('/output/generated_documentation_docs/:processId', async (req, res) => {
  ...
  try {
    ...
    const documentationFile = files.find(file => 
      file.Key.includes(processId) && file.Key.endsWith('documentation.json')
    );
    ...
    const url = await fileManager.s3.getSignedUrl('getObject', {
      ...
      ResponseContentDisposition: 'attachment; filename="documentation.json"'
    });
