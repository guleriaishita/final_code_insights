...
const OutputReview = () => {
  ...
  const [fileName, setFileName] = useState('comments.txt');

  useEffect(() => {
    const processId = sessionStorage.getItem('processId_comments');
    ...
    const fetchData = async () => {
      try {
        const url = `http://localhost:5000/api/output/generated_comments_docs/${processId}`;
        ...
        if (response.data.status === 'completed' && response.data.results) {
          ...
          setData({
            comments: response.data.results[firstFileName].comments,
  ...
  if (!data?.comments) return <div className="text-gray-600 p-4">No comments found</div>;
    ...
    <div className="min-h-screen">
      ...
      <main className="container mx-auto px-4 py-4">
        <div className="flex justify-between items-center mb-4">
          <h1 className="text-xl font-bold text-gray-900">Generated Comments</h1>
          <button 
        ...
        <div className="bg-white shadow rounded p-4">
          <ReactMarkdown 
            ...
            components={{
              ...
              inlineCode: props => <code className="px-1 rounded text-purple-600" {...props} />,
            }}
          >
            {data.comments}
          </ReactMarkdown>
